...
**Fully self-hosted on a DigitalOcean droplet as of 2026-07-28** (migrated off Railway + Supabase, which are now retired and no longer receive traffic — don't debug against them). Droplet `bali-server`, IP `143.198.179.150`, domain `bali.erasystems.com.ng`, root SSH access. Resized 2026-07-29 from 1GB to **2GB RAM / 50GB disk** after the original 1GB was found nearly saturated under normal load. Single docker-compose stack at `/opt/bali/docker-compose.yml`:
...
A new self-hosted staff dashboard (login-protected, replacing Retool — see "Admin visibility" section below) is being built to live at `bali.erasystems.com.ng`, since Retool's free tier can't do a custom domain or a simple custom login.
...
- Credentials live in `.env` at the project root (gitignored — never commit this file). `supabase/apply-schema.mjs` is re-runnable any time `schema.sql` changes (now points at the DigitalOcean Postgres via `DATABASE_URL`, not Supabase).
...
## Admin visibility — migrating from Retool to a new self-hosted dashboard (2026-07-29)

**Retool is being retired.** Its free tier can't do a custom domain (`bali.erasystems.com.ng`) or a simple admin/password login — both were explicitly requested — so a new lightweight dashboard is being built and self-hosted alongside the rest of the stack instead (own docker-compose service, direct Postgres access, session-based login with a changeable password). Check this section for whether it's live yet before assuming Retool (below) is still the active tool.

Old Retool dashboard (stale once the new one is live): **https://erasystems--bali-dashboard.retool.app** — org `erasystems`, resource "Bali Database" pointed at the OLD Supabase Postgres via the pooler (now retired — this resource would need repointing at the new DigitalOcean Postgres to keep working, which is moot once the replacement dashboard ships). Tabs for all 8 tables (bookings, contacts, invoices, contracts, conversations, pending_questions, knowledge_base, sops), built via Retool's AI app builder against the real schema. Retool's free-tier API tokens don't expose `resources`/`apps` scopes (only RPC/Custom Component Libraries), so this had to be built via direct browser automation rather than the REST API.
