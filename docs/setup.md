...
**Fully self-hosted on a Hetzner server as of 2026-08-01** (migrated off an earlier DigitalOcean droplet, which was itself migrated off Railway + Supabase — all three are retired and no longer receive traffic, don't debug against them). Server `bali-production`, IP `167.233.242.179`, domains `bali.erasystems.com.ng` (staff dashboard + PostgREST) and `n8n.erasystems.com.ng` (n8n editor/webhooks), root SSH access, 4 vCPU / 7.6GB RAM / 150GB disk (well above the resource pressure that forced a resize on the old DO droplet). Single docker-compose stack at `/opt/bali/docker-compose.yml`. The old DigitalOcean droplet (`bali-server`, `143.198.179.150`) is kept running as a rollback only — DNS no longer points at it, its `/opt/bali` files are a frozen snapshot from migration day, don't deploy to it or trust its state as current.

- **n8n** (`docker.n8n.io/n8nio/n8n`) — basic-auth admin + owner account, port 5678 localhost-only (reverse-proxied by Caddy). All 7 workflows active (01 inbound router, 02 sales flow, 03 PM toggle, 04 KB check, 05 fan-out, 06 privacy policy, 99 invoice/contract).
- **postgres** (16-alpine) — TWO databases: `n8n` (n8n's own state) and `bali` (the actual app schema, seeded from a real dump of the old Supabase data during migration — 12 tables: `contacts`, `bookings`, `conversations`, `knowledge_base`, `ingredients`, `recipes`, `recipe_ingredients`, `stock`, `sops`, `pending_questions`, `invoices`, `contracts`). Note: `ingredients`/`recipes`/`recipe_ingredients`/`stock` remain schema-only, unused by any current workflow or dashboard tab — their purpose isn't currently documented, worth clarifying with the owner if they're still needed.
- **postgrest** — drop-in Supabase REST replacement over the `bali` db, port 3000 internal. `SUPABASE_SERVICE_KEY` is now a custom-issued JWT (role `bali_service`, issuer `bali-self-hosted`), not a real Supabase key — `SUPABASE_URL=https://bali.erasystems.com.ng` still works unchanged since Caddy routes `/rest/v1/*` here. **Schema cache auto-reloads now** (fixed 2026-08-02, see `schema.sql`'s `pgrst_watch` event trigger) — a new/changed column used to need a manual `docker restart` of the postgrest containers before it was usable, silently 400'ing (`PGRST204`) until someone did. Applied on both `bali` and `bali_sandbox`; re-run `apply-schema.mjs` (or the event trigger block manually) against any future database that skips it.
- **gotenberg** (`gotenberg/gotenberg:8`) — PDF rendering, same as before, now just a local docker service instead of a separate Railway service. Renders the styled invoice HTML (`formatInvoiceHtml` in `stage3-4-action-code.js`) to a real PDF matching the actual `BALI-2026-003` template.
- **caddy** — reverse proxy + automatic HTTPS. Routes `/rest/v1/*` to postgrest, everything else to n8n. **This will change** once n8n moves to its own subdomain (`n8n.erasystems.com.ng`) to free up `bali.erasystems.com.ng` for the new staff dashboard (see below) — check this file's latest state before assuming the routing described here is still current.

The self-hosted staff dashboard (login-protected, replacing Retool — see "Admin visibility" section below) is **live** at `bali.erasystems.com.ng` (default handler; also reachable at `/dashboard`), running as its own docker-compose service (`bali-bali-dashboard-1`).

Retool (free tier, admin UI) and the old Railway/Supabase project are both stale/retired — don't point anything new at them.
...
- Credentials live in `.env` at the project root (gitignored — never commit this file). `supabase/apply-schema.mjs` is re-runnable any time `schema.sql` changes; `DATABASE_URL` points at `localhost:15432` and the script opens its own throwaway SSH tunnel to the Hetzner Postgres (bound to `127.0.0.1` on the server, not exposed publicly) — port 15432, not 5432, since this dev machine also runs an unrelated local Postgres on 5432. Note `schema.sql` uses plain `CREATE TABLE` (not `IF NOT EXISTS`), so it's only safe to run against an empty database, not a re-run against production as-is.
- **Meta webhook callback URL:** registered on **Nexa's** endpoint (see below), which forwards to Bali's `https://bali.erasystems.com.ng/webhook/whatsapp-inbound`. **This URL changes** once n8n moves to `n8n.erasystems.com.ng` — if you update it, you MUST also update Nexa's `BALI_WEBHOOK_URL` Railway variable and re-verify end-to-end (a past incident: `BALI_WEBHOOK_URL` pointing at the bare domain instead of the full `/webhook/whatsapp-inbound` path caused every customer message to silently 404 and drop). **Verify token:** value of `META_WEBHOOK_VERIFY_TOKEN` in `.env`.
...
# then PUT the changed file(s) to https://bali.erasystems.com.ng/api/v1/workflows/{id}
# using an X-N8N-API-KEY (generate via POST /rest/api-keys against an authenticated n8n
# owner session if you don't already have one) -- deactivate, PUT a minimal body of just
# {name, nodes, connections, settings}, then reactivate. Ask me to do this, I have the workflow IDs.
```
...
## Admin visibility — self-hosted dashboard (replaced Retool, live since before the Hetzner migration)

**Retool has been retired.** Its free tier couldn't do a custom domain (`bali.erasystems.com.ng`) or a simple admin/password login — both were explicitly requested — so the self-hosted dashboard below replaced it, running as its own docker-compose service (`bali-bali-dashboard-1`, direct Postgres access, session-based login with a changeable password). It's the default handler at `bali.erasystems.com.ng` (confirmed live, redirects to `/dashboard`).

Old Retool dashboard (now stale, kept here for reference only — don't use): **https://erasystems--bali-dashboard.retool.app** — org `erasystems`, resource "Bali Database" pointed at the OLD Supabase Postgres via the pooler, long since retired. Tabs for all 8 tables (bookings, contacts, invoices, contracts, conversations, pending_questions, knowledge_base, sops), built via Retool's AI app builder against the real schema.
...
2. **Add real staff contacts** beyond the PM (Joseph Owolabi, already in) — lawyer, and eventually HR/Procurement/Accounts/Event Assistant/Security/Supervisor/Facility Manager, via the self-hosted dashboard's Contacts tab.
3. **Populate `knowledge_base`** with real FAQ content via the self-hosted dashboard.
