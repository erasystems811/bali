...
- **postgrest** — drop-in Supabase REST replacement over the `bali` db, port 3000 internal. `SUPABASE_SERVICE_KEY` is now a custom-issued JWT (role `bali_service`, issuer `bali-self-hosted`), not a real Supabase key — `SUPABASE_URL=https://bali.erasystems.com.ng` still works unchanged since Caddy routes `/rest/v1/*` here.
