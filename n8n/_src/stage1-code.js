...
async function notifyPmOfCompletedIntake(booking) {
  ...
  const summaryLines = [
    ...
    // client_reference is literally the string "none" when the client said
    // they don't have an IG/TikTok/website (see the extraction prompt) --
    // that's a real, deliberately-answered "no", not a value worth
    // repeating back to the PM, so it's treated the same as not having one.
    `- ${booking.is_existing_client ? 'Experienced' : 'First-time'} client${booking.client_reference && booking.client_reference !== 'none' ? `, ${booking.client_reference}` : ''}`,
